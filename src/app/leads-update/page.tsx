  ...
  const handleToExcel = () => {
    ...
    if (filters.leadSource !== 'all') {
      const sourceReportData = leadSources
        ...
        .map(source => ({ 'Lead Source': source }));
      
      if (sourceReportData.length === 0) {
        toast({ variant: 'destructive', title: 'No Lead Sources Found', description: 'There are no lead sources to export.' });
        return;
      }

      const ws = XLSX.utils.json_to_sheet(sourceReportData);
      ws['!cols'] = [{ wch: 30 }];
      const wb = XLSX.utils.book_new();
